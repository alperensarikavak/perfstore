...
export default function RegisterPage() {
    ...
    const onSubmit = async (data) => {
        setServerError('');
        const res = await registerUser(data.username, data.email, data.password, data.pin);
        if (!res.success) {
            ...
            <div className="glass-panel" style={{ padding: '40px', width: '100%', maxWidth: '400px' }}>
                ...
                <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    ...
                    <div>
                        <input
                            type="password"
                            {...register('pin', { required: 'PIN gerekli', minLength: { value: 4, message: 'En az 4 rakam' } })}
                        ...
                        />
                        {errors.pin && <span style={{ color: '#ef4444', fontSize: '0.8rem' }}>{errors.pin.message}</span>}
                    </div>
